...
def train(experiments, batch_outdir, config, hyperparams={"max_depth": 10}):
    ...
    ### AGGREGATE ALL FEATURES/LABELS FROM EXPERIMENTS
    ...
    for group_id, exp in enumerate(experiments):
        ...
        feat_filepath = op.join(feat_subdir, config['features']['feature_file'])
        # Get feature mask from config
        feat_mask = config['features']['mask']
        
        # Read in feature CSV
        with open(feat_filepath, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                batch_Y.append(row['movement_type'].lower())

                # Only use features with config mask set to 1
                row_feats = [row[feat] for feat in feat_mask.keys() if feat_mask[feat]]
                batch_X.append(row_feats)
...
def predict(experiment, config):
    ...
    with open(feat_filepath, 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            exp_Y.append(row['movement_type'].lower())

            # Only use features with config mask set to 1
            row_feats = [row[feat] for feat in feat_mask.keys() if feat_mask[feat]]
            exp_X.append(row_feats)
    ...
    try:
        ...
        logging.warning("Failed to open classifier {}".format(model_path))
        return None
    ...
    track_fpaths = sorted(glob.glob(op.join(track_subdir, '*' + config['track_ext'])))
    ...
    for i in range(num_tracks):
        ...
        with open(op.join(get_exp_subdir('predict_dir', experiment, config), op.basename(track_fpaths[track_ID[i]])), 'w') as f:
...
def predict_batch_metrics(batch_true_Y, batch_pred_Y, batch_prob_Y, batch_alltracks, batch_outdir, config):
    ...
    if len(batch_true_Y):
        logging.info("{} of {} tracks have labels, calculating metrics".format(
