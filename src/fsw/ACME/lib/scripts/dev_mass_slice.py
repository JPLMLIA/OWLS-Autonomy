...
if __name__ == "__main__":
    ...
    parser.add_argument('--mass',               default=None,
                                                help='Mass to slice at')
    ...
    # take slice at specified mass
    slice_mass = float(args.mass)
    slice_index = find_nearest_index(mass_axis, slice_mass)

    # example run for later
    # python dev_mass_slice.py --data /data/MLIA_active_data/data_OWLS/ACME/lab_data/2020_CESI_Data/200904/pickle/001_Mix27_10uM.pickle --peak_csv /data/MLIA_active_data/data_OWLS/WORK_DIR/jake/outdir_acme/10161631_bg/001_Mix27_10uM/Unknown_Masses/001_Mix27_10uM_UM_peaks.csv --mugshot_dir /data/MLIA_active_data/data_OWLS/WORK_DIR/jake/outdir_acme/10161631_bg/001_Mix27_10uM/Unknown_Masses/Mugshots --mass 362.8
    ...
    ax_tr1.plot(time_axis, exp[slice_index, :], color='tab:blue', label="Original", linewidth=0.5)
    ax_tr1.set_title(f"Time Trace @ {slice_mass:.2f} amu/z")
    ax_tr1.set_ylabel("Ion Count")
    ax_tr1.set_xlabel("Time (min)")
    ax_tr1.set_ylim(0, tr_max)
    ax_tr1.legend()

    # Timetrace reconstruction statistics
    ax_tr2.plot(time_axis, exp_mugshot_only[slice_index, :], color='tab:orange', label="Mugshots", linewidth=0.5)
    ax_tr2.plot(time_axis, exp_nopeak_means[slice_index, :], color='tab:green', label="Summarized", linewidth=0.5)
    _std_min = exp_nopeak_means[slice_index, :] - exp_nopeak_stds[slice_index, :]
    _std_max = exp_nopeak_means[slice_index, :] + exp_nopeak_stds[slice_index, :]
    ax_tr2.fill_between(time_axis, _std_min, _std_max, color='tab:green', alpha=.5, label="±1σ")
    ax_tr2.set_ylabel("Ion Count")
    ax_tr2.set_xlabel("Time (min)")
    ax_tr2.set_ylim(0, tr_max)
    ax_tr2.legend()

    # Timetrace reconstruction sampled
    ax_tr3.plot(time_axis, exp_recon[slice_index, :], color='tab:purple', label="Sampled Data", linewidth=0.5)
    ax_tr3.set_ylabel("Ion Count")
    ax_tr3.set_xlabel("Time (min)")
    ax_tr3.set_ylim(0, tr_max)
    ax_tr3.legend()

    # Timetrace comparison
    ax_tr4.plot(time_axis, exp[slice_index, :], color='tab:blue', label="Original", linewidth=0.5)
    ax_tr4.plot(time_axis, exp_mugshot_only[slice_index, :], color='tab:orange', label="Mugshots", linewidth=0.5)
    ax_tr4.plot(time_axis, exp_nopeak_means[slice_index, :], color='tab:green', label="Summarized Mean", linewidth=0.5)
    ax_tr4.fill_between(time_axis, _std_min, _std_max, color='tab:green', alpha=.5, label="Summarized stddev")
    ax_tr4.set_ylabel("Ion Count")
    ax_tr4.set_xlabel("Time (min)")
    ax_tr4.set_ylim(0, tr_max)
    ax_tr4.legend()

    plt.tight_layout()

    fig.savefig("bg_mass_slice.png", dpi=800)