...
def load_dqe(dqe_file):
    """
    Loads individual DQE from CSV file

    Parameters
    ----------
    dqe_file: str
        path to CSV file containing DQE

    Returns
    -------
    dqe: float
        real-valued DQE entry from file, or None if an error occurs
    """

    with open(dqe_file, 'r') as f:
        reader = csv.DictReader(f)
        dqes = list(reader)

    if len(dqes) != 1:
        logger.warning(f'Unexpected {len(dqes)} != 1 entries in {dqe_file}')
        return

    dqe_dict = dqes.pop()

    if len(dqe_dict) != 1:
        logger.warning(f'Unexpected {len(dqe_dict)} != 1 entries in {dqe_file}')
        return

    dqe = list(dqe_dict.values())[0]
    return float(dqe)
