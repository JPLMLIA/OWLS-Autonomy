        ...
        products = [
            {
                ...
                'final_sue': base_utility * sue[cand_id] * dqe[cand_id],
                'initial_sue_per_byte': sue_per_byte[cand_id],
                'final_sue_per_byte': base_utility * sue_per_byte[cand_id] * dqe[cand_id],
...
def mmr(alpha, base_utility=1.0, similarity_func=DEFAULT_SIMILARITY):
    ...
    def prioritize(asdp_type, metadata):
        ...
        while len(candidates) > 0:
            ...
            dqe_cand = dqe[candidx]

            # Compute the MMR scores
            diversity_factors = (
                (1 - alpha) + (alpha * (1.0 - sim_max))
            )
            scores = sue_cand * diversity_factors * dqe_cand * base_utility
