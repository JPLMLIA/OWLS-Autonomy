...
def HIRAILS_analysis(config, experiments, steps, use_existing, log_name, log_folder, cores, space_mode, batch_outdir, priority_bin=0, manifest_metadata_file=None, manifest_metadata=None, note="", toga_config=None, kill_file=""):
    """Main logic function for the HIRAILS analysis pipeline"""
...
def main():
    ...
    parser.add_argument('--log_name',           default="HIRAILS_analysis_pipeline.log",
                                                help="Filename for the pipeline log. Default is HIRAILS_analysis_pipeline.log")
    ...
    # setup the plotter
    pltt = Plotter(save_to=op.join(args.log_folder, "HIRAILS_analysis_pipeline_memory.mp4"))
    globalQ = pltt.get_queues('HIRAILS_analysis_pipeline.py')

    # Set up the watcher arguments
    watch = {'HIRAILS_analysis_pipeline.py': {'queue': globalQ.graph, 'pid': os.getpid()}}

    # Start watcher then the plotter
    watcher(watch)
    pltt.start()

    HIRAILS_analysis(args.config,
                     args.experiments,
                     args.steps,
                     args.use_existing,
                     args.log_name, 
