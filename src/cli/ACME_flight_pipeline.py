...
def ACME_flight(data, outdir, masses, params, sue_weights, dd_weights, log_name, log_folder, cores, priority_bin, manifest_metadata, manifest_metadata_file, kill_file, knowntraces=False, noplots=True, noexcel=True, debug_plots=False, space_mode=True, saveheatmapdata=False):
    """
    ...
    if len(mp_batches) != 0:
        for b in mp_batches:
            if os.path.exists(kill_file):
                logging.error(f"Encountered kill file {kill_file}, halting operations. Remove file to re-enable pipeline usage")
            else:
...
def main():
    ...
    parser.add_argument('--log_name',           default="ACME_flight_pipeline.log",
    ...
    parser.add_argument('--kill_file',          default="ACME_flight_kill_file",
    ...
    # setup the plotter
    pltt = Plotter(save_to=op.join(args.log_folder, "ACME_flight_pipeline_memory.mp4"))
    globalQ = pltt.get_queues('ACME_flight_pipeline.py')

    # Set up the watcher arguments
    watch = {'ACME_flight_pipeline.py': {'queue': globalQ.graph, 'pid': os.getpid()}}
