...
def HIRAILS_ground(config, experiments, steps, use_existing, log_name, log_folder, cores, space_mode, batch_outdir, priority_bin=0, manifest_metadata_file=None, manifest_metadata=None, note="", kill_file=""):
    """ Main logic function for the HIRAILS ground data system """
...
def main():
    ...
    parser.add_argument('--log_name',           default="HIRAILS_ground_pipeline.log",
                                                help="Filename for the pipeline log. Default is HIRAILS_ground_pipeline.log")
    ...
    # setup the plotter
    pltt = Plotter(save_to=op.join(args.log_folder, "HIRAILS_ground_pipeline_memory.mp4"))
    globalQ = pltt.get_queues('HIRAILS_ground_pipeline.py')

    # Set up the watcher arguments
    watch = {'HIRAILS_ground_pipeline.py': {'queue': globalQ.graph, 'pid': os.getpid()}}

    # Start watcher then the plotter
    watcher(watch)
    pltt.start()

    HIRAILS_ground(args.config,
                   args.experiments,
                   args.steps,
                   args.use_existing,
                   args.log_name, 
