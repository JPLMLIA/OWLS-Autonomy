...
def ACME_analysis(data, outdir, masses, params, sue_weights, dd_weights, log_name, log_folder, cores, priority_bin, manifest_metadata, manifest_metadata_file, knowntraces, noplots, noexcel, debug_plots, space_mode, saveheatmapdata):
    """
...
def main():
    ...
    parser.add_argument('--log_name',           default="ACME_analysis_pipeline.log",
    ...
    # setup the plotter
    pltt = Plotter(save_to=op.join(args.log_folder, "ACME_analysis_pipeline_memory.mp4"))
    globalQ = pltt.get_queues('ACME_analysis_pipeline.py')

    # Set up the watcher arguments
    watch = {'ACME_analysis_pipeline.py': {'queue': globalQ.graph, 'pid': os.getpid()}}

    # Start watcher then the plotter
    watcher(watch)
    pltt.start()

    ACME_analysis(args.data, 
                  args.outdir, 
                  args.masses, 
                  args.params, 
                  args.sue_weights, 
