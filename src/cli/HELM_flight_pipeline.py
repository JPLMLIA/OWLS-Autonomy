...
def HELM_flight(config, experiments, steps, use_existing, log_name, log_folder, cores, predict_model, space_mode, batch_outdir, priority_bin=0, manifest_metadata_file=None, manifest_metadata=None, note="", kill_file="HELM_flight_kill_file"):
    """Main logic function for the HELM flight software"""
...
def main():
    ...
    parser.add_argument('--log_name',           default="HELM_flight_pipeline.log",
                                                help="Filename for the pipeline log. Default is HELM_flight_pipeline.log")
    ...
    parser.add_argument('--kill_file',          default="HELM_flight_kill_file",
    ...
    # setup the plotter
    pltt = Plotter(save_to=op.join(args.log_folder, "HELM_flight_pipeline_memory.mp4"))
    globalQ = pltt.get_queues('HELM_flight_pipeline.py')

    # Set up the watcher arguments
    watch = {'HELM_flight_pipeline.py': {'queue': globalQ.graph, 'pid': os.getpid()}}

    # Start watcher then the plotter
    watcher(watch)
    pltt.start()

    HELM_flight(args.config,
                args.experiments,
                args.steps,
                args.use_existing,
                args.log_name,
