...
def asdp_experiment(experiment, config):
    ...
    if not config['_field_mode']:
        visualization(experiment, config, "FAME", config['_cores'])
...
def main():
    ...
    parser.add_argument('--config',             default=op.join(op.abspath(op.dirname(__file__)), "configs", "fame_config.yml"),
                                                help="Path to configuration file. Default is cli/configs/fame_config.yml")
    ...
    parser.add_argument('--log_name',           default="FAME_pipeline.log",
                                                help="Filename for the pipeline log. Default is FAME_pipeline.log")
    ...
    # setup the plotter
    pltt = Plotter(save_to=op.join(args.log_folder, "FAME_pipeline_memory.mp4"))
    globalQ = pltt.get_queues('FAME_pipeline.py')

    # Set up the watcher arguments
    watch = {'FAME_pipeline.py': {'queue': globalQ.graph, 'pid': os.getpid()}}
