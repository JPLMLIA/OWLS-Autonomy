...
def create_dist_objs(**kwargs):
    ...
    if kwargs['distribution_name'] == 'truncnorm':
        ...
        for mu, sigma, min_val, max_val in zip(kwargs['mean'], kwargs['std'],
            ...
            # If there is no width to the distribution, create uniform dist that will return one number
            if (min_val-max_val) == 0:
                dist = stats.uniform(loc=mu, scale=max_val-min_val)
            # Otherwise, create truncated normal dist
            else:
...
def check_rect_overlap(rect_1, rect_2):
    """Test if two rectangles overlap

    rect_1: list
        Should be format [min_row, max_row, min_col, max_col]
    rect_2: list
        Should be format [min_row, max_row, min_col, max_col]
    """

    # Check rows for non-overlap
    if rect_1[1] < rect_2[0] or rect_1[0] > rect_2[1]:
        return False
    # Check cols for non-overlap
    if rect_1[3] < rect_2[2] or rect_1[3] > rect_2[2]:
        return False

    # Couldn't rule out overlap, therefore return that there is overlap
    return True


def count_overlaps_in_rect_list(rects):
    """Given a list of rectangles, count number of overlaps

    Each overlap is counted only once.

    rects: list
        Rectangles should be format [min_row, max_row, min_col, max_col]
    """

    overlaps = 0

    for r1, r2 in combinations(range(len(rects)), 2):
        overlaps += check_rect_overlap(rects[r1], rects[r2])
